<template>
	<div class="page" id="search-artists-page">
		<div class="page-header">
			<h1 class="page-title">{{ title }}</h1>
		</div>

		<div v-if="loading && !artists.length" class="loading">
			<LoadingSpinner />
		</div>
...
const title = computed(() => {
	if (query.value) {
		return `Исполнители: ${query.value}`;
	}
...
const loadData = async (linkParam?: string, nextFromParam?: string) => {
...
const loadMore = async () => {
	if (loadingMore.value || !hasMore.value || !more.value || !more.value.next_from) {
		return;
	}
