...
const albums = computed<TAlbum[]>(() => {
	...
	return playlistsData
		.filter(p => {
			...
			return p.context === "album" || p.context === "albums";
		})
		.map(playlist => ({
...
const totalCount = computed(() => {
	const playlistsData = data.value?.playlists || [];
	const albumsCount = playlistsData.filter(p => {
		if (p.playlist_id === -1) {
			return false;
		}
		return p.context === "album" || p.context === "albums";
...
const hasMore = computed(() => {
	return albums.value.length < totalCount.value;
});
...
const loadMore = async () => {
	...
	const result = await $fetch<{ count: number; playlists: TPlaylist[] }>(`/api/vk/playlists`, {
		params: {
			owner_id: ownerId.value,
			offset: data.value?.playlists.length || 0
		}
	}).catch((error) => {
	...
	if (result && data.value) {
		data.value.playlists.push(...result.playlists);
