...
- **Библиотека**: Сохранение любимых треков в персональную библиотеку
- **Плейлисты**: Создание и управление собственными плейлистами с возможностью добавления треков из любого раздела
- **Коллекции**: Просмотр музыкальных коллекций других пользователей
- **Текст песен**: Отображение текста песен во время воспроизведения
- **Загрузка музыки**: Скачивание треков для офлайн-прослушивания (доступно в десктопной версии)
...
```bash
docker compose logs -f meridius-dev
```

#### Остановка контейнера

```bash
docker compose stop meridius-dev
```

#### Перезапуск контейнера

```bash
docker compose restart meridius-dev
```

#### Остановка и удаление контейнера

```bash
docker compose down meridius-dev
```
...
```bash
docker compose logs -f meridius
```

#### Остановка production контейнера

```bash
docker compose stop meridius
```

#### Выполнение команд в контейнере

```bash
docker compose exec meridius-dev /bin/sh
```
...
```bash
# Очистка кэша Nuxt и node_modules
npm run clean
```
...
Убедитесь, что FFmpeg установлен и доступен в PATH, или укажите путь в переменной `FFMPEG_BINARY`.

### Проблемы с Tauri сборкой

1. Убедитесь, что Rust установлен: `rustc --version`
2. Проверьте системные зависимости для вашей ОС
...
3. Проверьте логи: `docker compose logs -f meridius-dev`

### Проблемы с сессиями

Убедитесь, что `NUXT_SESSION_PASSWORD` имеет минимум 32 символа и `NUXT_COOKIE_KEY` в правильном формате PEM.
